...
- **0.9.0** (2026-07-26): **TOWNS パッド RUN/SELECT の追加 + ジョイスティック仕様の実装追従**。
  - ジョイスティックの **TOWNS パッド RUN/SELECT ボタン** (note 21/22, §4.2.1) を追加。
    ...
    通常の方向ノートとは OR で重畳され、方向ノート単独では左右・上下が排他になる
    (SOCD ガード)。未対応ファームはこのノートを無視するだけなので後方互換
  - §4.2.1 のボタン番号割当表を **実装準拠に修正** (0.3.0 以降の実装は 1 始まりの
    割当を使用しており、旧表とずれていた)。右レバー (15-18)、MSX マウスボタン
    (19-20) を明記
  - パッドモード (SET_CONFIG key 0x03) に **3 = MSX マウス** を追記 (firmware v1.0.0
    で実装済み)。MSX マウスモードの CC 0x30/0x31 (移動量デルタ) の用法を §4.2.2 に明記
  - 互換性: ワイヤフォーマットは 0.8 と完全互換。後方互換の機能追加のため **MINOR up**
- **0.8.0** (2026-06-24): **トランスポート非依存化と BLE-MIDI バインディングの追加**。
  - プロトコルを「USB-MIDI 上」から「MIDI 1.0 メッセージ層の上」に再定義。
    リアルタイムメッセージ / SysEx の中身は一切変えず、下位トランスポートを
    差し替え可能にした (§2)
  - **BLE-MIDI トランスポート** (§2.3) を追加。標準 BLE-MIDI GATT サービス
    (`03B80E5A-EDE8-4B33-A751-6CE34EC4C700` / characteristic
    `7772E5DB-3868-4112-A1A9-F2669D106BF3`) を採用。BLE-MIDI パケットの
    タイムスタンプ header/low バイトを剥がして素の MIDI バイト列に復元すること、
    SysEx が複数 BLE パケットに分割される点の再結合を規定
...
Mimic X プロトコルは、**MIDI 1.0 メッセージ層**の上に構築されたレトロ PC 向け HID デバイス
制御プロトコルである。スマートフォン（ホスト）とマイコン（デバイス）間で、キーボード・
ジョイスティック・マウスなどの HID 操作を双方向にやりとりする。下位トランスポートは
USB-MIDI と BLE-MIDI のいずれでもよく、リアルタイムメッセージ / SysEx の中身は共通である
(§2)。

### 1.1. 設計原則

1. **MIDI 1.0 準拠** — 標準的な MIDI デバイス (USB-MIDI / BLE-MIDI) として認識される。特殊なドライバ不要
...
5. **トランスポート非依存** — リアルタイム / SysEx のワイヤフォーマットは下位トランスポートに依存しない。タイムアウト等のパラメータのみトランスポートごとに調整する (§2.4)
6. **汎用性** — Mimic X 以外のプロジェクトでも MIDI 制御プロトコルとして流用可能

### 1.2. 用語

| 用語 | 意味 |
|------|------|
| ホスト | スマートフォン（Flutter アプリ）。MIDI ホスト側 (USB-MIDI host / BLE-MIDI central) |
| デバイス | マイコン（CH32X035 / ESP32 等）。MIDI デバイス側 (USB-MIDI device / BLE-MIDI peripheral) |
| ターゲット | デバイスが模倣する対象のレトロ PC |

## 2. トランスポート

本プロトコルの上位レイヤ (§3 以降) は MIDI 1.0 のバイト列だけに依存し、下位トランスポート
には依存しない。現行で定義するトランスポートは USB-MIDI (§2.1) と BLE-MIDI (§2.3) の 2 つ。
タイムアウト等のトランスポート依存パラメータは §2.4 に集約する。
...
ESP32 等の無線アダプタは BLE-MIDI ペリフェラルとして動作する。標準 BLE-MIDI
(MIDI over Bluetooth Low Energy, Apple の MIDI-BLE 仕様) に準拠する。

- **GATT サービス UUID**: `03B80E5A-EDE8-4B33-A751-6CE34EC4C700`
- **MIDI I/O characteristic UUID**: `7772E5DB-3868-4112-A1A9-F2669D106BF3`
  (Write Without Response / Notify、双方向)
- ホストは BLE central、デバイスは BLE peripheral。デバイスは上記サービス UUID を
  advertise する。アドバタイズ名はデバイス名のヒントに使ってよいが、**正式な識別は
  接続後の `IDENTIFY_RESPONSE` の serial[16] で行う** (§6.4.0、トランスポート非依存)

#### 2.3.1. BLE-MIDI パケットのデコード

characteristic の各パケットは「ヘッダバイト + (タイムスタンプ low + MIDI バイト列)…」の
形式を持つ。上位レイヤに渡す前に **タイムスタンプ header / low バイトを除去して素の MIDI
バイト列に復元**すること。Running status と複数メッセージのパッキングに対応する。

#### 2.3.2. SysEx の分割と再結合

SysEx (`F0 … F7`) は 1 つの BLE-MIDI characteristic 書き込みに収まらない場合、複数パケットに
分割される。途中パケットの先頭にもタイムスタンプ low バイトが挟まり得るため、受信側は
`F0` から `F7` までを跨パケットで連結してから解釈する。`IDENTIFY_RESPONSE` /
`CAPABILITY_RESPONSE` は長くなりやすく、分割が常態となる前提で実装すること。

- 推奨: 接続後に **ATT_MTU を 247 へ拡張**し、connection interval を **7.5–15 ms** で
  ネゴシエートしてレイテンシと分割回数を抑える

### 2.4. トランスポート別パラメータ

ワイヤフォーマットは共通だが、レイテンシ特性が異なるため以下のパラメータはトランスポート
ごとに切り替える。ホストは接続中のトランスポートに応じて値を選ぶ。

| パラメータ | USB-MIDI | BLE-MIDI | 備考 |
|---|---|---|---|
| ACK / 応答タイムアウト (§6.1.4) | 100 ms | **400 ms** | BLE のラウンドトリップ + 分割を考慮 |
...
| 13 | Z | MD 6B モード |
| 14 | Mode | MD 6B モード |
| 15 | 右レバー 上 (Up2) | Libble Rabble モード |
| 16 | 右レバー 下 (Down2) | Libble Rabble モード |
| 17 | 右レバー 左 (Left2) | Libble Rabble モード |
| 18 | 右レバー 右 (Right2) | Libble Rabble モード |
| 19 | マウス左ボタン | MSX マウスモード |
| 20 | マウス右ボタン | MSX マウスモード |
| 21 | TOWNS RUN | ATARI モード (0.9.0+) |
| 22 | TOWNS SELECT | ATARI モード (0.9.0+) |
| 23-127 | 拡張ボタン | |

(0, 5, 8 は歴史的経緯により欠番)

**TOWNS RUN/SELECT (note 21/22) の意味論** (protocol 0.9.0+, ATARI モードのみ):

FM TOWNS のパッドは RUN ボタン = 左右同時アサート、SELECT ボタン = 上下同時アサート
として配線されている。デバイスは note 21 (RUN) 押下中は左・右の両出力ピンを、
note 22 (SELECT) 押下中は上・下の両出力ピンをアサートする。通常の方向ノート
(1-4) の状態とは **OR で重畳**される。

一方、RUN/SELECT がアサートされていない間は、方向ノートの左右 (3, 4) と上下 (1, 2)
はデバイス側で**排他** (SOCD ガード) となる: 両方が押下状態の場合、対応する 2 つの
出力ピンは**どちらも非アサート** (ニュートラル) にする。これはホスト側の入力デバイス
由来の一瞬の同時押しが TOWNS パッド対応ソフトに RUN/SELECT と誤解釈されるのを防ぐ
ためである。意図的な左右/上下同時アサートは必ず note 21/22 で表現すること。

RUN/SELECT 未対応のファーム (protocol 0.8 以前) は note 21/22 を無視する。
...
**MSX マウスモードでの CC 0x30/0x31** (firmware v1.0.0+): パッドモードが
MSX マウス (3) の間、CC 0x30/0x31 は移動量デルタ (マウスと同じオフセット表現:
64 = 0, 0 = -64, 127 = +63) として解釈され、デバイス内の累積デルタに加算される。
スティック絶対値としては扱われない。
...
- 推奨タイムアウト: トランスポート依存 — USB-MIDI **100 ms** / BLE-MIDI **400 ms** (§2.4)
...
- ホストは ACK の応答を確認する。**連続失敗が閾値に達した場合** (USB-MIDI: 3 回 = 3 秒 /
  BLE-MIDI: 5 回 = 5 秒、§2.4)、ホストは接続失敗と判断してユーザーに通知し、デバイス一覧に戻る
- デバイスも同様に一定時間 HEART_BEAT を受信しなければ WAITING に戻る (上記と同じ閾値)
...
| 0x03 | ジョイスティック パッドモード | 0=ATARI, 1=MD 6B, 2=Libble Rabble (XPD-1LR), 3=MSX マウス |
