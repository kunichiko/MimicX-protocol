...
### 5.1. LED 状態通知 (Channel 14)

ターゲット機側からの LED 制御信号をホストに通知する。

```
Control Change, Channel 14
  control = LED カテゴリ (下表参照)
  value   = LED 状態 (0=消灯, 127=点灯)
```

| CC 番号 | LED |
|---------|-----|
| 0x00 | LED 0 (機種依存: 例 X68000 "かな") |
| 0x01 | LED 1 (機種依存: 例 X68000 "ローマ字") |
| 0x02 | LED 2 (機種依存: 例 X68000 "コード入力") |
| 0x03 | LED 3 (機種依存: 例 X68000 "CAPS") |
| 0x04 | LED 4 (機種依存: 例 X68000 "INS") |
| 0x05 | LED 5 (機種依存: 例 X68000 "ひらがな") |
| 0x06 | LED 6 (機種依存: 例 X68000 "全角") |
| 0x07-0x7F | 予約 |
...
ホスト (スマホアプリ)                デバイス (マイコン)
  ...
  │                                    │
  │←── CC (CH14, CC0x03, val=127) ────│  CAPS LED 点灯通知
  │                                    │
```
